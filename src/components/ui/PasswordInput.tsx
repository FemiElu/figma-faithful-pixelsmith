...
const PasswordInput: React.FC<PasswordInputProps> = ({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex h-20 items-center border border-neutral-400 bg-white relative px-5 rounded-3xl">
        <input
          type={showPassword ? "text" : "password"}
          className={cn("w-full text-[#7A7A7A] text-lg outline-none", className)}
          {...props}
        />
        <button
          type="button"
          onClick={togglePasswordVisibility}
          className="absolute right-[20px] w-[24px] h-[24px]"
